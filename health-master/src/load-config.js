...
module.exports = () => {
  const configPath = process.env.CONFIG_PATH || path.resolve(__dirname, '..', 'config-files', 'config.json');
  ...
  const envMapping = {
    ...
    'down-scale-cooldown-ms': process.env.DOWN_SCALE_COOLDOWN_MS
  };

  for (const key in envMapping) {
    if (envMapping[key]) {
      data[key] = envMapping[key];
      if (key.endsWith('-ms')) data[key] = parseInt(envMapping[key]);
    }
  }
