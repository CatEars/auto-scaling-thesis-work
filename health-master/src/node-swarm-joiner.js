...
const initializeDroplet = async (droplet, swarmJoinCommand, userId, userToken) => {
  ...
  // In the following strings, be very careful with the usage of '," and `
  const sshConf = `-o "StrictHostKeyChecking=no"`;
  const copyRepo = `scp ${sshConf} -r ${dropletCopyPath} ${ip}:/health`;
  const onDropletCommand = `cd /health && ./deploy-script.sh '${userId}' '${userToken}' '${swarmJoinCommand}'` +
        `'${dockerUsername}' '${dockerPassword}'`
  const deployCommand = `ssh ${sshConf} ${ip} "${onDropletCommand}"`;
  const fullCommand = `${copyRepo} && ${deployCommand}`;
  const { stdout, stderr } = await exec(fullCommand);
  if (stderr) return 'fail';
  return 'success';
}
...
const removeDropletFromSwarm = async (droplet) => {
  if (_.includes(droplet.tags, 'static')) return;
  const flavor = getDropletFlavor(droplet);
  const swarmLeader = flavor['swarm-leader'];
  const hostname = droplet.name;
  const removeCommand = `docker node ls | grep Down | cut -d' ' -f 1 | xargs docker node rm`
  const sshCommand = `ssh ${swarmLeader} "${removeCommand}"`;
  ...
  for (let i = 0; i < retries; ++i) {
    try {
      ...
      console.log('When removing from swarm we got the following output:');
      console.log(stdout);
      break;
    } catch (err) {
      console.log('Removing droplet from swarm errored');
      console.error(err);
    }
...
const main = async () => {
  ...
  stream.on('data', (data) => {
    ...
    } else if (new_val === null && old_val &&
               old_val.status === 'active' && old_val.isInitialized) {
      console.log('Trying to remove the droplet');
      console.log(old_val);
      removeDropletFromSwarm(old_val);
    }
